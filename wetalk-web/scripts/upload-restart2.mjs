...
conn.on('ready', async () => {
  ...
  // 2. start.sh —— 加 ES autoconfigure exclude
  const EXCLUDE = 'org.springframework.boot.autoconfigure.data.elasticsearch.ElasticsearchDataAutoConfiguration,org.springframework.boot.autoconfigure.data.elasticsearch.ElasticsearchRepositoriesAutoConfiguration,org.springframework.boot.autoconfigure.elasticsearch.ElasticsearchRestClientAutoConfiguration';
...
`;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (i % 10 === 0 && i > 0) {
      const err = await exec('tail -20 /opt/wetalk/wetalk.log 2>/dev/null | grep -E "ERROR|Caused|Exception|Started" | head -5');
      if (err.stdout.trim()) log('    LOG:', err.stdout.trim());
    }
    await sleep(5000);
  }
  ...
  if (nginxTest.code === 0) {
    ...
    await exec('rm -f /etc/nginx/sites-enabled/default 2>/dev/null; nginx -t 2>&1 && (systemctl start nginx 2>&1 || nginx 2>&1) && echo NGINX_OK');
  }
