...
conn.on('ready', async () => {
  ...
  // 3. 写 start.sh —— 不需要 exclude 参数了，application.yml 已内置
...
`;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (i % 5 === 0 && i > 0) {
      const err = await exec('tail -30 /opt/wetalk/wetalk.log 2>/dev/null | grep -E "ERROR|Caused|Exception|Started|Description|Action" | head -5');
